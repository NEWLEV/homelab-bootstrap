...
<body>
  ...
  <script>
    ...
    function renderDigest() { if (!state.digest) return; const d = state.digest; els.digest.innerHTML = `<div>${d.tasks} tasks, ${d.auto_fixes} auto-fixes, ${d.prs_opened} PRs opened, ${d.pending_approvals} approvals waiting, ${d.failures} failures</div><ul class="digest-list">${(d.notable_events || []).map((event) => `<li>${event.title} ? ${event.agent}</li>`).join('')}</ul>`; }
