...
test_hermes_compose_contract() {
    ...
    assert_contains "hermes compose allows optional operations rag" 'OPS_RAG_URL: ${HERMES_OPS_RAG_URL:-}' "$output"
    ...
    assert_contains "hermes compose masks openssl arm cpu probing" 'OPENSSL_armcap: ${HERMES_OPENSSL_ARMCAP:-0}' "$output"
