...
test_compose_contract() {
    ...
    assert_contains "local rag keeps operations API loopback-only" 'published: "8091"' "$local_rag_output"
    assert_contains "local rag makes operations source configurable" 'AISHA_OPS_SOURCE' "$local_rag_output"
    assert_contains "manifest keeps operations RAG optional" '"name": "local-rag-ops-api"' "$manifest_output"
    assert_contains "manifest does not require operations RAG" '"required": false' "$manifest_output"
